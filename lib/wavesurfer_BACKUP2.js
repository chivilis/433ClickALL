...
var WaveSurfer = {
    ...
    drawBuffer: function () {
        var nominalWidth = Math.round(
            this.getDuration() * this.params.minPxPerSec * this.params.pixelRatio   //GENUINE
            //this.getDuration() * this.backend.buffer.length                           //CUSTOM
        );
        ...
        if (this.params.partialRender) {
            ...
            //make single array of peak indexes IF STEREO
            ...
            this._playAudioLikeBuffer = function() {
                ...
                console.log('peaks in %: ' + peakPerc);
                //console.log(this._loadedClickTracks)
                //console.log(this._loadedClickTracks[0])
    ...
    getStepToReduceBuffer: function() {
        var sampleRate = this.backend.buffer.sampleRate;
        var duration = this.backend.buffer.duration;
        return Math.round(sampleRate / duration);
    },
    ...
    load: function (url, peaks, preload) {
        ...
        switch (this.params.backend) {
            ...
            case 'MediaElement': (function() {
                console.log('Media files are not supported yet!');
            })()       
        }
    },
    ...
    loadBuffer: function (url, peaks) {
        /*
        this._loadedClicks = [];
        for(var track in url) {
            this._loadedClicks.push(url[track]);
        }
        */
        this._trackMeta = {};
        this._loadedClickTracks = [];
        
        this._trackMeta = {
            url: url,
            filename: (function(){return url.substring(url.lastIndexOf('/')+1, url.length )})()
        }
        this._loadedClickTracks.push(this._trackMeta)
        ;
        //console.log(this._loadedClicks);
