        ...
        if (this.params.partialRender) {
            ...
            for(self.i = 0; self.i < this.backend.buffer.numberOfChannels; self.i++) {
                ...
                peaks.forEach(function(peak) {
                    ...
                    peak > 0.7 ? self._tempPeakIndexes[self.i].push(Array.prototype.indexOf.call(self._tempBuffer[self.i], peak)) : null
                });
                
            }
            this._tempPeakIndexes = self._tempPeakIndexes;
            self = null;
            ...
            this._playAudioLikeBuffer = function() {
                ...
                function initiateTimeOut(i) {
                  setTimeout(function() { doStuff(i) }, .1);
                }
                function doStuff(i) {
                    console.log(i);
                    i++;
                    if (i <= self.backend.buffer.length) {
                        initiateTimeOut(i); 
                    }
                }
                ...
                for(var sample in this._audioLikeBuffer) {
                    
                       
                        
                    setTimeout(function() {
                        if(this._audioLikeBuffer[sample]) {
                         //function getRandomIntInclusive(min, max) {
                            self.r= Math.floor(Math.random() * (self.max - self.min + 1)) + min;
                            self.g= Math.floor(Math.random() * (self.max - self.min + 1)) + min;
                            self.b= Math.floor(Math.random() * (self.max - self.min + 1)) + min;
                        }

                    //$(body).css("background-color":"rgb(211,111,120)");
