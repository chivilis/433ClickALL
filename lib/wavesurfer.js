...
var WaveSurfer = {
    ...
    makeAudioLikeArr: function() {
        ...
        for(var i = 0; i < this.backend.buffer.length; i++) {
            self.i = i;
            for(var index in this._tempPeakIndexes) {
                if(self.i == this._tempPeakIndexes[index]) {
                    this._audioLikeBuffer[self.i] = true;
                } 
            }
            
        }
        Object.create(this._audioLikeBuffer); 
        console.log(this._tempPeakIndexes)
        
        /*
        this._audioLikeBuffer = this._tempPeakIndexes;
        for(var i in this._audioLikeBuffer) {
            if(this._audioLikeBuffer[i] === false) {
                this._audioLikeBuffer[i].push(0);
            }
        }
